...
BEGIN
	...
	UPDATE PATIENT.tb_VACCINE_PATIENT SET IS_DELETED=1 WHERE PATIENT_ID=@local_id_patient AND VACCINE_ID=@local_vaccine_id;
END
...
AS
BEGIN
	DECLARE @local_old_vaccine_id INT=(SELECT VACCINE_ID FROM ADMINISTRATOR.tb_VACCINE WHERE VACCINE_TYPE=@param_OLD_VACCINATION_TYPE);
